...
import { basePublicClient, mainnetPublicClient, optimismClient } from "@/common/utils/walletClients";
...
async function isNounHolder(userAddress: Address, userId: number, castHash: string): Promise<boolean> {
  return isNftBalanceAboveThreshold(mainnetPublicClient, "0x9c8ff314c9bc7f6e59a9d9225fb22946427edc03", userAddress, 0);
}

async function isPurpleDaoHolder(userAddress: Address, userId: number, castHash: string): Promise<boolean> {
  return isNftBalanceAboveThreshold(mainnetPublicClient, "0xa45662638e9f3bbb7a6fecb4b17853b7ba0f3a60", userAddress, 0);
}
...
function MintConditionsNotMetComponent({ checkPayload }: ConditionsNotMetComponentProps): ReactElement {
  ...
  return (
    <div tw="flex flex-row w-full h-full text-[52px] p-[64px]" style={{ backgroundColor: "#FBCB07", color: "black" }}>
